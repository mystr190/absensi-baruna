...
// ==========================================
// FITUR OVERVIEW DASHBOARD ANALYTICS REALTIME
// ==========================================
...
function handleGetOverviewStats() {
  try {
    ...
    if (sheetSiswa && sheetSiswa.getLastRow() > 1) {
      const data = sheetSiswa.getRange(2, 1, sheetSiswa.getLastRow() - 1, 5).getValues();
      for (let i = 0; i < data.length; i++) {
        const nama = String(data[i][2] || '').trim();
        if (!nama) continue;
        totalSiswa++;
        const gRaw = String(data[i][4] || '').trim().toUpperCase();
        const isP = gRaw === 'P' || gRaw.startsWith('PEREMPUAN');
        if (isP) totalPerempuan++; else totalLaki++;

        const kls = String(data[i][3] || 'Lainnya').trim();
        if (!kelasMap[kls]) kelasMap[kls] = { kls: kls, total: 0, L: 0, P: 0 };
        kelasMap[kls].total++;
        if (isP) kelasMap[kls].P++; else kelasMap[kls].L++;
      }
    }

    // 2. Presensi Siswa Hari Ini (LogAbsen)
    let studentAbsenSummary = { Hadir: 0, Sakit: 0, Izin: 0, Alpa: 0, Terlambat: 0 };
    const sheetLog = ss.getSheetByName(SHEET_LOG);
    if (sheetLog && sheetLog.getLastRow() > 1) {
      const logData = sheetLog.getRange(2, 1, sheetLog.getLastRow() - 1, 6).getValues();
      for (let i = logData.length - 1; i >= 0; i--) {
        const rawDate = logData[i][0];
        if (!rawDate) continue;
        if (getFormattedDate(rawDate) !== todayStr) continue;
    ...
    const sheetPelanggaran = ss.getSheetByName(SHEET_PELANGGARAN);
    if (sheetPelanggaran && sheetPelanggaran.getLastRow() > 1) {
      const pData = sheetPelanggaran.getRange(2, 1, sheetPelanggaran.getLastRow() - 1, 8).getValues();
      for (let i = 0; i < pData.length; i++) {
        const pel = String(pData[i][7] || 'Lainnya').trim();
        if (pel) {
          totalViolations++;
          violationSummary[pel] = (violationSummary[pel] || 0) + 1;
        }
      }
    }

    // 4. Presensi Guru Hari Ini (LogAbsenGuru & PengajuanIzin Disetujui)
    let guruAbsenSummary = { Hadir: 0, Sakit: 0, Izin: 0, Alpa: 0, DinasLuar: 0 };
    const guruPresensiSet = new Set();

    // a. Tarik dari LogAbsenGuru
    const sheetLogGuru = ss.getSheetByName(SHEET_LOG_GURU);
    if (sheetLogGuru && sheetLogGuru.getLastRow() > 1) {
      const lgData = sheetLogGuru.getRange(2, 1, sheetLogGuru.getLastRow() - 1, 7).getValues();
      for (let i = lgData.length - 1; i >= 0; i--) {
        const rawDate = lgData[i][2];
        if (!rawDate) continue;
        if (getFormattedDate(rawDate) !== todayStr) continue;

        const uname = String(lgData[i][3] || '').trim().toLowerCase();
        const nama = String(lgData[i][4] || '').trim().toLowerCase();
        const stUpper = String(lgData[i][5] || '').trim().toUpperCase();
        const ketLower = String(lgData[i][6] || '').trim().toLowerCase();

        if (uname) guruPresensiSet.add(uname);
        if (nama) guruPresensiSet.add(nama);
    ...
    // b. Tarik dari PengajuanIzin (Disetujui hari ini yang belum tercatat di LogAbsenGuru)
    const sheetIzin = ss.getSheetByName(SHEET_PENGAJUAN_IZIN);
    if (sheetIzin && sheetIzin.getLastRow() > 1) {
      const izData = sheetIzin.getRange(2, 1, sheetIzin.getLastRow() - 1, 9).getValues();
      for (let i = 0; i < izData.length; i++) {
        const izTgl = getFormattedDate(izData[i][5]);
        const izSt = String(izData[i][8] || '').trim().toLowerCase();

        if (izTgl === todayStr && izSt === 'disetujui') {
          const uname = String(izData[i][2] || '').trim().toLowerCase();
          const nama = String(izData[i][3] || '').trim().toLowerCase();
          const katUpper = String(izData[i][6] || 'IZIN').trim().toUpperCase();

          if (!guruPresensiSet.has(uname) && !guruPresensiSet.has(nama)) {
            if (uname) guruPresensiSet.add(uname);
            if (nama) guruPresensiSet.add(nama);
    ...
    // 5. Guru & Staf Belum Absen
    const unabsenGuruList = [];
    const sheetUsers = ss.getSheetByName(SHEET_USERS);
    if (sheetUsers && sheetUsers.getLastRow() > 1) {
      const uData = sheetUsers.getRange(2, 1, sheetUsers.getLastRow() - 1, 5).getValues();
      for (let i = 0; i < uData.length; i++) {
        const uRole = String(uData[i][3] || '').trim().toLowerCase();
        if (uRole === 'admin' || uRole === 'administrator' || uRole === 'siswa') continue;

        const uUsername = String(uData[i][1] || '').trim();
        const uNama = String(uData[i][4] || uUsername).trim();
        const uUsernameLower = uUsername.toLowerCase();
        const uNamaLower = uNama.toLowerCase();

        if (!guruPresensiSet.has(uUsernameLower) && !guruPresensiSet.has(uNamaLower)) {
          unabsenGuruList.push({
            username: uUsername,
            nama: uNama,
            role: String(uData[i][3] || 'Guru')
          });
        }
      }
    }

    return jsonResponse('success', 'Overview stats ditarik.', {
      totalSiswa: totalSiswa,
      totalLaki: totalLaki,
      totalPerempuan: totalPerempuan,
      totalKelas: Object.keys(kelasMap).length,
      ...
      totalViolations: totalViolations,
      violationSummary: violationSummary,
      totalGuruLog: guruPresensiSet.size,
      guruAbsenSummary: guruAbsenSummary,
      unabsenGuruList: unabsenGuruList
    });
  } catch (err) {
    return jsonResponse('error', 'Gagal ambil overview stats: ' + err.toString());
  }
}
